...
BIN_NAME=go-lang-test

VERSION := $(shell grep "const Version " version.go | sed -E 's/.*"(.+)"$$/\1/')
GIT_COMMIT=$(shell git rev-parse HEAD)
GIT_DIRTY=$(shell test -n "`git status --porcelain`" && echo "+CHANGES" || true)
IMAGE_NAME := "SaberZA/go-lang-test"

default: build

help:
	@echo 'Management commands for go-lang-test:'
	@echo
	@echo 'Usage:'
...
build-docker:
	@echo "building ${BIN_NAME} ${VERSION}"
	docker build -t go-lang-test:build -f Dockerfile.build .
	docker run --name=go-lang-test -v $(GOPATH):/gopath/  go-lang-test:build
	docker rm -f go-lang-test
